...
### Custom Components (e.g. subnet)

Some resource types require extra naming components. Use one `custom_component` block per component:
...
On every `terraform plan` and `terraform refresh`, the provider calls the Admin API with the stored `id` to verify the name still exists. If the name has been deleted outside of Terraform, the provider removes the resource from state so that Terraform plans a recreation on the next `apply`.

This requires `admin_password` to be configured (via the `PROACTNAMING_ADMIN_PASSWORD` environment variable or the provider `admin_password` argument).
